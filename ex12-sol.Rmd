---
title: "Exercise Set 12 Solutions"
author: "ETC3550/ETC5550: 2021"
output:
...
Now we will use that ARIMA(5,1,2)(2,0,0)[7] model and modify the knot until the AICc is optmized.
...
augment(elec_model) %>%
  features(.innov, ljung_box, dof = 12, lag = 21)
```

The model passes the residual tests and seems to give reasonable forecasts. The residuals look like they have some heteroskedasticity, but otherwise look ok.
...
```{r ex5c}
gg_tsresiduals(fit)
augment(fit) %>% features(.innov, ljung_box, dof = 18, lag = 24)
```
